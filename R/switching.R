...
switching <- function(epoch_list){
    ...
    for(i in seq_along(epoch_list)){
        for(j in 1:5){
            for(t in 2:(dim(epoch_list[[i]])[1])){
                dist1 = mean((epoch_list[[i]][t, joints[[j]][1], 1:2] - new_list[[i]][t-1, joints[[j]][1], 1:2])^2, na.rm=TRUE)
                dist2 = mean((epoch_list[[i]][t, joints[[j]][2], 1:2] - new_list[[i]][t-1, joints[[j]][1], 1:2])^2, na.rm=TRUE)
                if(dist2 < dist1){
