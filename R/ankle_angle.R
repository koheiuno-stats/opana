...
#' @param stand bool.
#' @param position "SmallToe" or "BigToe" or Both. 
#'
#' @export

ankle_angle <- function(data_array, stand=TRUE, LR="Left", position="SmallToe"){
    ...
    for(t in 1:(dim(data_array)[1])){
        ...
        if(is.na(K2A[1])){next}        
        H2T = apply(data_array[t, paste0("L",position), 1:2], 2, mean,na.rm=TRUE) -
            data_array[t, "LHeel", 1:2]        
        if(is.na(H2T[1])){next}
        ...
        out_pro = H2T[1]*K2A[2] - H2T[2]*K2A[1]            
        if(out_pro >= 0){
        ...
        H2T = apply(data_array[t, paste0("R", position), 1:2], 2, mean,na.rm=TRUE) -
            data_array[t, "RHeel", 1:2]
        if(is.na(H2T[1])){next}
